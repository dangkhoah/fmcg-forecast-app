...
import { Download, TrendingUp, Filter, Loader2, Search, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, RotateCcw, Save, Eye, X, Trash2, Check, Calendar, Edit2, BarChart3, LineChart, Share2, Dot, CircleOff, Info, Zap, Timer, Maximize2, Minimize2 } from 'lucide-react';
...
export default function Forecast() {
  ...
  const handleViewHistory = (h) => {
    ...
    toast.success(`Loaded forecast from ${new Date(h.created_at).toLocaleString()}`);
  };
  ...
  const handleForecast = async () => {
    ...
    try {
      ...
      toast.error(errorMessage || 'Forecast failed - Check console for details');      
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '24px', marginBottom: '24px' }}>
        <div className={`card ${restoredFlash ? 'restored-flash' : ''}`} style={{ minHeight: '650px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              ...
              <button onClick={handleClearCache} className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '4px' }} title="Clear model cache">
        ...
        <div 
          className="card" 
          style={{ 
            position: isChartExpanded ? 'fixed' : 'relative', 
            overflow: isChartExpanded ? 'auto' : 'hidden', 
            minHeight: isChartExpanded ? '100vh' : '650px',
            top: isChartExpanded ? 0 : 'auto',
            left: isChartExpanded ? 0 : 'auto',
            width: isChartExpanded ? '100vw' : 'auto',
            height: isChartExpanded ? '100vh' : 'auto',
            zIndex: isChartExpanded ? 1000 : 1,
            borderRadius: isChartExpanded ? 0 : '12px',
            margin: 0,
        ...
        >
          ...
          <div style={{ 
            ...
            alignItems: 'center', 
            marginBottom: activeHistoryId && result ? 8 : 16,
            padding: isChartExpanded ? '20px 40px 0' : 0
          }}>
          ...
          {activeHistoryId && result && (
            <div style={{
              ...
              padding: '8px 12px', marginBottom: 16, borderRadius: '6px', margin: isChartExpanded ? '0 40px 16px' : '0 0 16px',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: '13px', color: '#4338ca', fontWeight: 500 }}>
                  Viewing: {result.name || `Forecast from ${new Date(result.created_at).toLocaleString()}`}
                </span>
              </div>
              <button
          ...
          {chartData ? (
            <div style={{ padding: isChartExpanded ? '0 40px' : 0 }}>
              <div style={{ height: isChartExpanded ? 'calc(100vh - 220px)' : '480px' }}>
                {chartType === 'line' ? (
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: isChartExpanded ? 32 : 16, justifyContent: 'center' }}>
              ...
              {(result?.training_time !== undefined && result?.training_time !== null) && (
                <div style={{ textAlign: 'center', marginTop: isChartExpanded ? 24 : 12, fontSize: '11px', color: '#94a3b8', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
