...
import toast from 'react-hot-toast'; // Import Eye icon
import { Download, TrendingUp, Filter, Loader2, Search, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, RotateCcw, Save, Eye } from 'lucide-react';
...
export default function Forecast() {
  ...
  const [displayedForecast, setDisplayedForecast] = useState(null); // New state for the currently displayed forecast
  ...
  // Reset selected product filter when the displayed forecast changes
  useEffect(() => {
    setSelectedProduct('');
  }, [displayedForecast]);
  ...
  const handleReset = () => {
    ...
    setDisplayedForecast(null); // Also reset displayed forecast
    setResult(null);
    setSearchTerm('');
  ...
  const handleForecast = async () => {
    ...
    try {
      ...
      setDisplayedForecast(res.data); // Set the newly run forecast as the displayed one
      setResult(res.data);
  ...
  const productIds = useMemo(() => {
    if (!displayedForecast?.detailed_records) return []; // Use displayedForecast
    return [...new Set(displayedForecast.detailed_records.map(r => r.product_id))].sort((a, b) => a - b);
  }, [displayedForecast]); // Dependency on displayedForecast

  const filteredData = useMemo(() => {
    if (!displayedForecast?.detailed_records) return null; // Use displayedForecast
  ...
  }, [displayedForecast, selectedProduct, aggregation]); // Dependency on displayedForecast

  const tableRecords = useMemo(() => {
    if (!displayedForecast?.detailed_records) return []; // Use displayedForecast
    let filtered = displayedForecast.detailed_records; // Use displayedForecast
    ...
    return filtered; // Dependency on displayedForecast
  }, [displayedForecast, selectedProduct, searchTerm, sortConfig]);
  ...
  const chartData = displayedForecast ? { // Use displayedForecast
    labels: filteredData ? filteredData.labels : displayedForecast.dates, // Use displayedForecast
    datasets: [
      { 
        label: `${selectedProduct ? `Product ${selectedProduct}` : 'All Products'} (${aggregation === 'mean' ? 'Mean' : 'Sum'})`, 
        data: filteredData ? filteredData.values : displayedForecast.values, // Use displayedForecast
        borderColor: '#4338ca', 
        backgroundColor: 'rgba(67,56,202,0.1)', 
        fill: true, 
        tension: 0.3 
      },
      ...((displayedForecast.lower_bound && !selectedProduct && aggregation === 'mean') ? [{ // Use displayedForecast
        label: 'Upper Bound', data: displayedForecast.upper_bound, borderColor: '#94a3b8', borderDash: [5, 5], pointRadius: 0, fill: false, // Use displayedForecast
      }] : []),
      ...((displayedForecast.lower_bound && !selectedProduct && aggregation === 'mean') ? [{ // Use displayedForecast
        label: 'Lower Bound', data: displayedForecast.lower_bound, borderColor: '#94a3b8', borderDash: [5, 5], pointRadius: 0, fill: false, // Use displayedForecast
      }] : []),
    ],
  } : null;
  ...
  return (
    <div>
      ...
      <div className="grid-2">
        ...
        <div className="card" style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            {displayedForecast && result && displayedForecast.id !== result.id && ( // Show button if a historical forecast is displayed
              <button
                onClick={() => setDisplayedForecast(result)}
                className="btn btn-secondary"
                style={{ padding: '4px 8px', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '4px' }}
              >
                <RotateCcw size={14} /> View Current Forecast
              </button>
            )}
            {displayedForecast && productIds.length > 0 && ( // Use displayedForecast
      ...
      {displayedForecast && displayedForecast.detailed_records && ( // Use displayedForecast
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, justifyContent: 'flex-end' }}>
              ...
              <span style={{ fontSize: '14px', color: '#64748b', whiteSpace: 'nowrap' }}>
                Showing {tableRecords.length} of {displayedForecast.detailed_records.length} rows
              </span>
            </div>
          </div>
            ...
            <tbody>
              {history.map((h) => (
                <tr key={h.id}>
                  ...
                  <td style={{ width: '1%' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn btn-secondary" onClick={() => setDisplayedForecast(h)} title="View Forecast">
                        <Eye size={16} />
                      </button>
