# need to save this file under UTF-8-BOM in order for emoji characters appear correcty
...
Write-Host ""
Write-Host "Streaming logs to this terminal and $backendErr, $modelErr, ..." -ForegroundColor Cyan
...
try {
    ...
    try {
        ...
        Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile","-ExecutionPolicy","Bypass","-File","`"$scriptPath`""  -WorkingDirectory $PSScriptRoot
