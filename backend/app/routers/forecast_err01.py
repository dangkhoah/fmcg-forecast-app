...
async def run_forecast(
    ...
    try:
        model_result = await call_forecast_model(dataset.id, model_input)
...
async def create_scenario(
    ...
    model_input = {
        "file_path": actual_path,
        "force_retrain": False, # Scenarios typically don't force retrain
        **payload.parameters
    }
    try:
        model_result = await call_forecast_model(dataset.id, model_input)
...
async def clear_forecast_cache(current_user: User = Depends(get_current_user)):
    """
    Manually clears the in-memory forecast model cache.
    """
    forecast_model_cache.clear()
    return {"detail": "Model cache cleared successfully"}
@router.get("/policy")
